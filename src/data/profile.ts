...
export const CAREERS: Career[] = [
	{
		...
		bullets: [
			...
			'주임급 개발자의 React 학습과 프로젝트 리드를 지원해 모바일·태블릿 전용 렌즈 계산기 운영 반영 후 유지보수까지 독립적으로 맡을 수 있도록 역할을 확장'
		]
	},
	{
...
export const TROUBLES = [
	{
		label: '외부 API 연동 장애',
		title: '서버 로그를 직접 읽어 보안 정책 차단을 특정',
		text: '본인인증 요청이 응답 없이 멈춰 애플리케이션 코드와 서버 설정 중 원인을 구분해야 했습니다. 로그와 보안 설정을 확인해 요청이 애플리케이션에 도달하기 전 차단되는 원인을 특정하고 서비스를 우선 정상화한 뒤, 백엔드팀의 정식 보안 개선으로 연결했습니다.'
	},
	{
		label: '배포 3일 전 기기 변경',

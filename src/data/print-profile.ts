...
export const PRINT_SKILLS: PrintSkill[] = [
	{
		label: '코어',
		value: 'React, TypeScript, JavaScript, JSP'
	},
	{
		label: '상태 관리',
	...
	{
		label: '협업',
		value: 'Git, Jira, Confluence, Claude Code'
	}
];
...
export const PRINT_TARGET_RESUMES: Record<PrintTargetResumeId, PrintTargetResume> = {
	senior: {
		...
		intro:
			'6명 규모 프론트엔드 파트에서 업무를 나누고 온보딩과 기술 의사결정을 맡아 레거시 전환과 운영 개선을 함께 이끌어요. 화면만 보지 않고 입력·포커스·반응형·배포·보안 설정까지 원인을 좁혀 해결하고, 판단 근거를 문서로 남겨 팀이 같은 기준으로 개발하게 해요.',
		sentences: [
			'6명 규모 프론트엔드 파트에서 업무를 나누고 온보딩과 기술 의사결정을 맡아 레거시 전환과 운영 개선을 함께 이끌어요.',
			'화면만 보지 않고 입력·포커스·반응형·배포·보안 설정까지 원인을 좁혀 해결하고, 판단 근거를 문서로 남겨 팀이 같은 기준으로 개발하게 해요.',
			'JSP 레거시를 React로 전환하고 화면·정책·배포 구조를 분리해요.',
			'모바일·태블릿 입력, 포커스, 반응형 기준을 사용자 피드백에 맞춰 조정해요.'
		],
		strengths: [
		...
		works: [
			{
				...
				role: 'IA 관리, 업무 배분, 기획·디자인 의사결정과 요구사항 우선순위 조율',
				scope: '권한별 정책 분석, 공통 컴포넌트·렌즈 계산 구조 설계, 주요 화면 구현',
				problem:
					'10년 가까이 쌓인 레거시라 제품 판매·결제 화면은 손대기 어려울 만큼 위험하고, 권한별 정책도 코드 곳곳에 흩어져 있어요. 프론트엔드 수정 요청은 잦은데 서버와 함께 배포해야 하고, 배너·팝업 운영 조건까지 코드에 들어 있어 일정이 바뀔 때마다 다시 배포해야 해요. 키보드와 스캐너에 따라 LOT 입력 오류도 반복돼요.',
				process: [
					'화면부터 옮기면 정책 누락이 따라와서, 권한별 페이지와 업무 정책을 먼저 문서로 정리하고 모든 파트가 같은 기준으로 개발하게 했어요. UI가 서로 다른 렌즈 계산기 세 개는 화면별로 다시 구현하지 않고 입력·포커스·검증 로직만 화면과 분리한 공통 구조로 묶었어요.',
					'배너·팝업은 코드 수정 대신 관리자가 직접 바꾸는 방식으로 바꿔서, 재배포 부담과 수기 입력 실수를 함께 줄이도록 기획·백엔드와 조율했어요. LOT 값은 입력 단계마다 막지 않고 서버로 보내기 직전 한 곳에서 변환·정제했어요.'
				],
				effect:
					'제품 판매 화면을 React 구조로 분리해 서버와 따로 배포할 수 있게 하고, 렌즈 계산기 3개의 입력·검증 로직을 하나로 통합해 400 오류와 중복 구현을 줄였어요. 배너·팝업 일정 변경은 프론트엔드 재배포 없이 처리하고, LOT 값도 입력 환경과 무관하게 서버 규격으로 전달해요.',
				takeaway:
					'기능을 옮기기보다 정책과 입력 규칙을 먼저 공통화해야 전환 과정에서 누락과 재작업을 줄일 수 있다는 점을 확인했어요.',
				effectHighlights: [
			...
			},
			{
				...
				role: '프로젝트 리드를 주임급 팀원에게 위임하고, 초기 기술 스택·UI·UX 결정과 기술 로직을 검토',
				scope: '초기 기술 스택·UI·UX 결정, 기술 로직 검토, 주문 가능 제품·상세 화면과 API 연동',
				problem:
					'개인정보 수집 동의를 받기 전에는 사용자의 도수 정보를 서버에 저장할 수 없다는 제약이 있어요. 여기에 운영 배포를 3일 앞두고 지급 기기가 교체됐고, 새 기기의 논리적 뷰포트가 모바일 크기로 인식되면서 기존 레이아웃까지 깨졌어요.',
				process: [
					'서버에 저장할 수 없는 조건이라 입력 중인 도수를 브라우저에 보관했고, 탭을 이동하거나 페이지를 새로고침해도 입력값이 유지되도록 Zustand의 상태 저장 기능을 사용했어요.',
					'화면 폭을 임의로 늘려 레이아웃만 맞추지 않고, 화면 배율에 따라 브라우저가 인식하는 너비를 확인해 태블릿이 모바일로 분류된 원인을 찾았어요. 이를 바탕으로 반응형 기준을 조정하고, 도수 기호를 입력할 수 없는 iOS 제약에는 선택 팝업으로 우선 대응했어요. 이후 사용자의 불편 피드백을 반영해 도수 기호를 직접 입력할 수 있는 커스텀 입력 컴포넌트를 만들고 유효성 검사를 강화했어요.'
				],
				effect:
					'운영 배포 3일 전 지급 기기가 바뀌었지만, 반응형 기준을 조정해 예정된 운영 일정에 맞춰 오픈했어요. 개인정보 동의 전에도 입력값을 잃지 않고 상담을 계속할 수 있도록 브라우저 기반 상태 저장 기능을 구현했어요.',
				takeaway:
					'실사용 기기와 브라우저 제약을 먼저 확인해야 화면 크기와 입력 방식의 문제를 실제 운영 환경 기준으로 풀 수 있다는 점을 배웠어요.',
				effectHighlights: ['예정된 운영 일정에 맞춰 오픈', '브라우저 기반 상태 저장 기능'],
			...
			},
			{
				...
				problem:
					'서버에서 교육 대화·문제·단계 데이터를 시작 시점에 한 번에 내려주는 구조라, 깊게 중첩된 데이터를 화면 단계별로 나눠 관리해야 했어요. Context API와 Funnel을 함께 사용하면서 화면이 반복해서 렌더링되고, 새로고침하면 진행 상태가 사라지는 문제도 있었어요.',
				process: [
					'상태 관리를 Zustand로 통합해 한 번에 전달된 데이터를 대화·문제·단계별로 관리하고, 새로고침 후에도 진행 상태가 유지되도록 구성했어요. 이후 한곳에 데이터가 몰려 비대해진 store는 도메인별로 다시 분리했어요.',
					'모달을 열 때 교육 대화가 초기화되는 문제는 진행 상황을 브라우저에 저장해 우선 대응하고, 모달 라이브러리를 도입해 화면 전체가 다시 렌더링되는 원인을 없앴어요. 2차 작업을 마친 뒤에는 기능과 기술 맥락을 문서로 정리해 후속 개발자에게 넘겼어요.'
				],
				effect:
					'서버에서 한 번에 받은 교육 데이터를 단계별로 관리하고, 새로고침이나 모달 진입 후에도 진행 상태가 유지되도록 구현했어요.',
				takeaway:
					'상태 관리 방식은 화면 구조보다 먼저 정리해야 하고, 한 곳에 몰린 상태는 도메인 기준으로 다시 나눠야 유지보수가 가능하다는 것을 확인했어요.',
				effectHighlights: ['교육 데이터를 단계별로 관리', '진행 상태가 유지되도록 구현'],
			...
			},
			{
				...
				problem:
					'휴대폰 본인인증 라이브러리를 적용하던 중 인증 요청이 응답 없이 멈췄어요. 원인이 애플리케이션 코드인지 서버 보안 설정인지 구분되지 않았어요.',
				process: [
					'백엔드 개발자와 함께 서버 로그와 보안 설정 파일을 확인해, 요청이 애플리케이션에 도달하기 전 Spring Security의 CSRF 정책에서 차단되는 원인을 직접 특정했어요.',
					'해당 경로 전체의 보안을 해제하면 위험 범위가 넓어진다고 판단해, 연동 개발을 계속하기 위한 임시 조치만 적용했어요. 특정 요청만 예외 처리하는 정식 개선 방향은 백엔드 개발자와 협의했어요.'
				],
				effect:
					'본인인증 라이브러리 연동을 이어갈 수 있도록 임시 조치를 적용하고, 특정 요청에만 정식 보안 예외를 적용하도록 백엔드 개발자와 협의했어요.',
				takeaway:
					'응답이 막힌 문제는 애플리케이션 코드만 보지 말고 서버 보안 정책까지 내려가야 원인을 정확히 좁힐 수 있었어요.',
				effectHighlights: [
					'라이브러리 연동을 이어갈 수 있도록 임시 조치를 적용',
					'정식 보안 예외를 적용하도록 백엔드 개발자와 협의'
				],
		...
		works: [
			{
				...
				problem:
					'5년 넘게 쌓인 대시보드는 백엔드와 프론트엔드가 한 프로젝트에 묶여 있어, 프론트엔드만 바꿔도 서버를 함께 배포해야 했고 프론트엔드 개발자를 뽑기도 어려웠어요. 대표와 PM은 JSP와 React를 동시에 운영하면 오히려 관리가 복잡해질 수 있다고 우려했어요.',
				process: [
					'운영과 유지보수 측면에서 React가 필요한 이유와 기능별 전환 방식을 제안했고, 합의 후 주요 관리자 기능을 옮겼어요. 한 차례 배포 직후 무한 호출이 발생했을 때는 먼저 롤백한 뒤 오류와 리다이렉트 흐름을 추적해 백엔드 URL 문제를 찾았어요.'
				],
				effect:
					'약 1년 동안 기존 서비스를 멈추지 않고 JSP와 React를 함께 운영하며 기능별로 화면을 옮겼어요.',
				takeaway:
					'마이그레이션은 한 번에 바꾸는 작업이 아니라, 롤백과 동시 운영이 가능한 구조를 만드는 일이 핵심이라는 점을 확인했어요.',
				effectHighlights: ['기존 서비스를 멈추지 않고', '기능별로 화면을 옮김'],
			...
			},
			{
				...
				process: [
					'표시 개수를 제한하면 관리자가 전체 분포를 볼 수 없어, 가까운 근무지를 하나로 묶어 보여주고 확대하면 나뉘는 방식을 앱·웹 팀과 논의해 우선 배포했어요. 국내와 해외가 서로 다른 지도 서비스를 써서 묶는 방식은 각각 구현하되, 관리자가 보는 화면과 탐색 흐름은 동일하게 맞췄어요.'
				],
				effect:
					'마커가 많은 근무지도 화면이 멈추지 않았고, 관리자는 지도에서 작업 현황을 다시 확인할 수 있었어요.',
				takeaway:
					'대량 데이터 화면은 렌더링 개수를 억지로 줄이기보다, 사용자가 전체 분포를 읽을 수 있는 클러스터링 기준을 세우는 편이 낫다는 점을 배웠어요.',
			...
			},
			{
				...
				problem:
					'국내외 고객의 정기결제를 인보이스로 수기 처리하고 있어, 고객이 요금제와 결제수단을 직접 등록하고 관리할 수 있는 기능이 필요했어요.',
				process: [
					'국내는 아임포트, 해외는 Stripe로 나눠 연동했어요. Stripe 문서만으로 확인하기 어려운 결제 상태는 고객센터에 직접 문의해 검증하고, 상태별 프론트엔드·서버 처리 범위를 백엔드 개발자와 합의했어요.'
				],
				effect:
					'국내외 고객이 제품 안에서 요금제와 결제수단을 직접 등록하고 관리할 수 있도록 정기결제 기능을 구현했어요.',
				takeaway:
					'결제 기능은 문서 확인만으로 끝나지 않고, 외부 지원과 서버 역할 분담까지 맞춰야 안정적으로 운영할 수 있었어요.',
				effectHighlights: ['직접 등록하고 관리할 수 있도록', '정기결제 기능을 구현'],
		...
		works: [
			{
				...
				period: '2018.07 - 2020.11',
				team: '고객사별 SI 프로젝트 팀',
				role: '관리자·회원·주문·결제 화면과 API 개발, 신규 팀원 가이드 작성',

...
export type PrintTargetResumeId = 'senior' | 'millie' | 'product' | 'aden';
...
export interface PrintTargetResume {
	 ...
	 * (예: 밀리의서재는 인재상 문서가 합니다체라 카드도 합니다체로 맞춘다)
	 */
	workText?: Record<
		string,
		Partial<
...
export const PRINT_TARGET_RESUMES: Record<PrintTargetResumeId, PrintTargetResume> = {
	senior: {
		...
		intro:
			'운영 중인 레거시 서비스를 React로 옮겨온 7년 차 프론트엔드 개발자예요. 기능을 만드는 데서 끝내지 않고, 사용자가 어떤 순서로 움직이는지까지 봐요. 초기 UI·UX는 기획자, 디자이너, 서버 개발자와 이야기하면서 같이 정해요. 리더로서는 제가 다 만들기보다, 팀원이 스스로 맡아 끝낼 수 있게 도와요.',
		introHighlights: [
			'7년 차 프론트엔드 개발자',
			'사용자가 어떤 순서로 움직이는지',
			'스스로 맡아 끝낼 수 있게'
		...
		sentenceHighlights: ['6명 규모 프론트엔드 파트', '판단 근거를 문서로 남겨'],
		strengths: [
	...
	millie: {
		...
		label: '밀리의서재 FE',
		headline:
			'레거시를 멈추지 않고 현대화하며, 상태·렌더링 구조와 장애 감지 체계를 함께 세우는 프론트엔드 개발자',
		intro:
			'운영 중인 레거시 서비스를 React로 옮겨온 7년 차 프론트엔드 개발자입니다. 서비스를 멈추지 않고 기능 단위로 옮기며, 롤백할 수 있는 구조를 먼저 만듭니다. 깊게 중첩된 데이터와 반복 렌더링은 상태를 나누는 기준부터 다시 세워 해결합니다. 판단의 근거와 실패한 시도까지 문서로 남겨, 다음 사람이 같은 자리에서 다시 막히지 않게 합니다.',
		introHighlights: [
			'7년 차 프론트엔드 개발자',
			'롤백할 수 있는 구조',
			'판단의 근거와 실패한 시도까지 문서로 남겨'
		],
		sentences: [
			'JSP 레거시를 두 번 React로 옮기며, 서비스를 멈추지 않고 기능 단위로 전환하고 롤백과 동시 운영이 가능한 구조를 만들었습니다.',
			'Context와 Funnel로 화면이 반복 렌더링되던 문제는 Zustand store를 도메인 기준으로 다시 나눠 단계별 상태를 정리했습니다.',
			'웹뷰 오류와 API 실패를 추적해 팀이 상주하는 Slack으로 공유하고, 사용자 문의를 기다리지 않고 대응을 시작하는 흐름을 만들었습니다.',
			'마커 2만 개 지도처럼 화면이 멈추는 문제는 렌더링 개수를 줄이기보다 데이터를 묶는 기준을 세워 해결했습니다.'
		],
		sentenceHighlights: [
			'롤백과 동시 운영이 가능한 구조',
			'Zustand store를 도메인 기준으로 다시 나눠',
			'사용자 문의를 기다리지 않고'
		],
		strengths: [
			'JSP 레거시를 두 차례 React로 점진 전환하고 롤백·동시 운영 구조까지 완결',
			'Context·Funnel의 반복 렌더링을 Zustand store 재설계로 정리하고 도메인별로 분리',
			'웹뷰 크래시·API 오류를 팀에 공유해 장애 대응 시점을 앞당김'
		],
		workIds: [
			'virtual-fitting',
			'acuvue-renewal',
		...
		// 인재상 문서가 합니다체라 대표 경험 카드도 같은 문체로 맞춘다. 사실은 그대로다.
		workText: {
			'acuvue-renewal': {
				problem:
					'10년 가까이 쌓인 레거시라 제품 판매·결제 화면은 손대기 어려울 만큼 위험했고, 권한별 정책도 코드 곳곳에 흩어져 있었습니다. 프론트엔드 수정 요청은 잦은데 서버와 함께 배포해야 했고, 배너·팝업 운영 조건까지 코드에 들어 있어 일정이 바뀔 때마다 다시 배포해야 했습니다. 키보드와 스캐너에 따라 LOT 입력 오류도 반복됐습니다.',
				process: [
					'화면부터 옮기면 정책 누락이 따라오므로, 권한별 페이지와 업무 정책을 먼저 문서로 남겨 모든 파트가 같은 기준으로 개발하게 했습니다. UI가 서로 다른 렌즈 계산기 세 개는 화면별로 다시 구현하지 않고, 입력·포커스·검증 로직만 화면과 분리한 공통 구조로 묶었습니다.'
				],
				effect:
					'제품 판매 화면을 React 구조로 분리해 서버와 따로 배포할 수 있게 하고, 렌즈 계산기 3개의 입력·검증 로직을 하나로 통합해 400 오류와 중복 구현을 줄였습니다. 배너·팝업 일정 변경은 프론트엔드 재배포 없이 처리하고, LOT 값도 입력 환경과 무관하게 서버 규격으로 전달합니다.',
				takeaway:
					'기능을 옮기기보다 정책과 입력 규칙을 먼저 공통화해야 전환 과정에서 누락과 재작업을 줄일 수 있다는 점을 확인했습니다.'
			},
			'virtual-fitting': {
				problem:
				...
				takeaway:
					'상태 관리 방식은 화면 구조보다 먼저 정리해야 하고, 한 곳에 몰린 상태는 도메인 기준으로 다시 나눠야 유지보수가 가능하다는 것을 확인했습니다.'
			},
			'shopl-migration': {
				problem:
				...
				process: [
					'운영과 유지보수 측면에서 React가 필요한 이유와 기능별 전환 방식을 제시해 진행 결정을 이끌었고, 합의 후 주요 관리자 기능을 옮겼습니다. 한 차례 배포 직후 무한 호출이 발생했을 때는 먼저 롤백한 뒤 오류와 리다이렉트 흐름을 추적해 백엔드 URL 문제를 찾았습니다.'
				],
				effect:
					'약 1년 동안 기존 서비스를 멈추지 않고 JSP와 React를 함께 운영하며 기능별로 화면을 옮겼습니다.',
				effectHighlights: ['기존 서비스를 멈추지 않고', '기능별로 화면을 옮겼습니다'],
				takeaway:
					'마이그레이션은 한 번에 바꾸는 작업이 아니라, 롤백과 동시 운영이 가능한 구조를 만드는 일이 핵심이라는 점을 확인했습니다.'
			},
			'shopl-map': {
				process: [
				...
				takeaway:
					'대량 데이터 화면은 렌더링 개수를 억지로 줄이기보다, 사용자가 전체 분포를 읽을 수 있는 클러스터링 기준을 세우는 편이 낫다는 점을 배웠습니다.'
			}
		}
	},
	product: {
	...
	aden: {
		...
		label: '에이든랩 FE',
		headline:
			'운영 중인 서비스를 멈추지 않고 React로 옮기며, 재사용 구조와 입력 검증을 함께 세우는 프론트엔드 개발자',
		intro:
			'운영 중인 레거시 서비스를 React로 옮겨온 7년 차 프론트엔드 개발자예요. 서비스를 멈추지 않고 기능 단위로 옮기고, 롤백할 수 있는 상태를 먼저 만들어요. 화면마다 흩어지던 입력·검증 로직은 공통 구조로 묶어요. 기획자, 디자이너, 서버 개발자와 이야기하면서 판단 기준을 문서로 남겨요.',
		introHighlights: ['7년 차 프론트엔드 개발자', '롤백할 수 있는 상태', '공통 구조로 묶어요'],
		sentences: [
			'JSP 레거시를 두 번 React로 옮기면서, 서비스를 멈추지 않고 기능 단위로 전환하고 롤백과 동시 운영이 가능한 구조를 만들어요.',
			'Zustand와 TanStack Query로 상태를 나누고, 화면마다 다시 구현하던 입력·검증 로직은 공통 컴포넌트로 묶어 중복을 줄여요.',
			'마커 2만 개 지도처럼 화면이 멈추는 문제는 렌더링 개수를 줄이기보다 데이터를 묶는 기준을 세워 풀어요.',
			'모바일·태블릿 입력, 포커스, 반응형 기준을 사용자 피드백에 맞춰 조정해요.'
		],
		sentenceHighlights: [
			'서비스를 멈추지 않고 기능 단위로 전환',
			'공통 컴포넌트로 묶어',
			'데이터를 묶는 기준'
		],
		strengths: [
			'JSP 레거시를 두 차례 React로 점진 전환하고 롤백·동시 운영 구조 구성',
			'렌즈 계산기 3개의 입력·검증 로직을 공통 구조로 통합해 중복 구현 제거',
			'대량 데이터 화면과 모바일·태블릿 입력 환경의 성능·반응형 기준 정리'
		],
		workIds: ['acuvue-renewal', 'smartfitting', 'shopl-migration', 'shopl-map', 'cre8orclub-sentry']
	}
};

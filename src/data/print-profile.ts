...
export const PRINT_TARGET_RESUMES: Record<PrintTargetResumeId, PrintTargetResume> = {
	senior: {
		...
		intro:
			'두 차례의 JSP 서비스 React 전환을 제안과 합의부터 구현까지 이끌었고, 모바일·태블릿 웹 제품을 개발했습니다. 화면에 드러난 현상만 고치지 않고 상태·입력·배포·서버 설정까지 원인을 좁힌 뒤, 판단 근거를 문서로 남겨 파트 전체가 같은 기준으로 개발하게 합니다.',
		strengths: [
			'React 전환의 필요성과 기능별 이전 순서를 대표·PM에게 제안해 합의를 만들고, 약 1년간 기존 서비스를 멈추지 않고 옮긴 경험',
			'UI가 서로 다른 렌즈 계산기 3개를 입력·검증 공통 구조로 묶고 권한별 레거시 정책을 문서화해, 중복 구현과 정책 누락을 함께 제거',
			'사용자 문의를 기다리지 않도록 웹뷰 오류·크래시를 Slack으로 알리는 모니터링을 직접 구축하고, 사용자 피드백을 반영해 입력 컴포넌트를 다시 설계',
			'신입 4명이 각자 운영 기능을 맡고 주임급 개발자가 프로젝트를 운영 반영까지 리드하도록, 경험에 맞춰 역할을 넓힌 온보딩 운영'
		],
		workIds: [
			'acuvue-renewal',
			'shopl-migration',

...
export const projects: Project[] = [
	{
		...
		result:
			'기능별 결합도를 낮추고 운영 배포 시간을 약 20분에서 1분 30초로 단축함. 배너 변경을 프론트 배포 없이 어드민에서 처리할 수 있게 됨.',
		resumeImpact: {
			label: '운영 배포 약 20분 → 1분 30초',
			detail:
				'60개 이상 화면과 15,000줄 제품판매 JavaScript를 React로 전환하고 배너 운영을 어드민으로 이관'
		},
		bullets: [
			'FE 3명의 업무를 배분하고 6개 역할별 라우팅, 결제 성공·실패, 인증·보안, S3 정적 리소스 규칙을 문서화',
			'15,000줄 제품판매 JavaScript를 기능 단위 React 구조로 전환해 운영 배포 시간을 약 20분에서 1분 30초로 단축',
	...
	},
	{
		...
		period: '2026.03',
		role: '프론트엔드 파트 리더',
		contribution: 'FE 3명 · 대시보드 기여도 10%',
		tags: ['웹 프로젝트', '코드'],
		stack: ['Sentry', 'Slack', 'React'],
		summary: 'Cre8or-Club 웹뷰 전체의 에러·성능을 Sentry 대시보드로 모니터링',
		problem:
			'웹뷰 장애를 CS 접수나 기획팀의 수동 모니터링 이후에야 인지해 대응이 늦었고, 프로필 첫 화면은 원본 GIF·고용량 이미지 로딩으로 FSP가 3초를 초과했음.',
		solution:
			'Web Vitals, HTTP 실패율, 화면·플랫폼별 오류와 영향 사용자 수를 추적하는 Sentry 대시보드를 구성하고 Slack 알림을 연동함. 프로필 이미지는 서버의 최적화 썸네일을 우선 사용하도록 변경함.',
		result:
			'수동 제보 전에도 오류를 빠르게 인지하는 대응 체계를 마련하고, 프로필 첫 화면 FSP를 3초 이상에서 1.3초 이하로 개선함.',
		bullets: [
			'LCP·INP·CLS, HTTP 실패율, 화면·플랫폼별 오류, 영향 사용자 수와 미처리 오류를 한 대시보드에서 추적',
			'Slack 알림을 연동해 CS 접수나 수동 모니터링 전에 이슈를 인지하고 대응',
			'원본 GIF·고용량 이미지 대신 최적화 썸네일을 우선 사용해 프로필 첫 화면 FSP를 3초 이상에서 1.3초 이하로 개선'
		]
	},
	{
		id: 'legacy-react',

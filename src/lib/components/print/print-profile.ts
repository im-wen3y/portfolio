...
export const PRINT_TARGET_RESUMES: Record<PrintTargetResumeId, PrintTargetResume> = {
	senior: {
		id: 'senior',
		label: '시니어 FE',
		headline: '운영 중인 서비스를 멈추지 않고 구조를 개선하는 프론트엔드 개발자',
		intro:
			'두 차례의 JSP 서비스 React 전환과 모바일·태블릿 제품 개발을 수행했습니다. 화면에 드러난 현상만 고치지 않고 상태·입력·배포·서버 설정까지 원인을 좁혀, 팀이 이후에도 관리할 수 있는 구조로 정리합니다.',
		strengths: [
			'JSP와 React를 함께 운영하며 기능 단위로 전환한 경험',
			'상태·입력·검증 로직을 화면에서 분리해 재사용 가능한 구조로 설계',
			'서버 로그와 보안 설정까지 확인해 장애 원인을 직접 특정'
		],
		workIds: [
			'acuvue-renewal',
			'virtual-fitting',
